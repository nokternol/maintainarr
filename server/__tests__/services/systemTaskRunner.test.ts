...
describe('SystemTaskRunner', () => {
  describe('dispatch', () => {
    it('dispatches system:identity-resolution to the identity job', async () => {
      const identityJob = makeIdentityJob();
      const runner = new SystemTaskRunner({ createIdentityJob: () => identityJob });
  ...
  describe('provider resolution (default job factory)', () => {
    beforeEach(async () => {
    ...
    it('resolves the active Radarr provider and populates identities; absent Plex does not throw', async () => {
      ...
      await expect(runner.run('system:identity-resolution')).resolves.toBeUndefined();
