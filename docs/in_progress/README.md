...
handlers, routes, services, domain logic, and jobs), sharing via public-API barrels plus a small
`server/kernel/`, dependency direction following the product loop
...
| **3** | Server layering (providers) | `modules/providers/` owns connections, roles, factory, settings service, task enablement, identity job — behind one barrel | Relocation |
...
| **5** | Server layering (mediaQueries + automations) | `modules/mediaQueries/` owns `MediaQueryService`; `modules/automations/` owns its services and the scheduler | Relocation |
| **6** | Server layering (auth + system + settings) | `modules/auth/` owns authService + session store; `modules/system/` merges both `health` homes + `systemTaskRunner`; `server/services/`, `jobs/`, `cron/`, `domain/`, `health/` are gone | Relocation |
| **7** | Server layering (closure) | Barrel-only imports enforced by an automated check; North Star doc promoted to `docs/architecture/`; ledger entry moves to Healed | Enforcement |
...
`jobs/identityResolutionJob.ts` + `jobs/identityJobFactory.ts`. Define `index.ts` as the public barrel
(roles, factory, descriptor types, settings service). The existing `providers.handler/routes/schemas`
stay — the module now owns its logic as the North Star prescribes.
...
`search/`. Barrel exports: `MediaItem`, `Normalized*`, `MEDIA_RULES`/`getRule`/descriptor projection,
the engine. `automations` and `mediaQueries` consume only the barrel.

## Phase 5 — mediaQueries + automations modules

`services/mediaQueryService.ts` → `modules/mediaQueries/`. `services/automationService.ts`,
`automationExecutor.ts`, `automationRunService.ts`, `combinationEvaluator.ts`,
`cron/automationScheduler.ts` → `modules/automations/`. Verify the dependency direction holds:
`automations` imports only the `media`, `mediaQueries`, `providers` barrels and kernel.

## Phase 6 — auth, system, settings
...
and `server/utils/` (emptied across Phases 2–5) deleted — empty directories are the phase's proof.
...
- An automated import-boundary check (lint rule or a small arch test walking the dependency graph)
  fails CI on any cross-module import that bypasses a barrel or violates the declared direction.
