...
| **4** | `phase-4-client-query-alignment.md` | The server **rule catalogue** is the single authority (`MediaRule = MediaRuleDescriptor & { predicate }`, mirroring `ActuatorTask`); the client **derives** its filter controls from projected, provider-gated `MediaRuleDescriptor`s and holds no catalogue (`FILTER_FIELDS` deleted); the UI shows the engine-backed preview count. | P1 (engine + real preview) | TDD (server naming/projection, then client hooks) + impeccable (filter view visual) |
