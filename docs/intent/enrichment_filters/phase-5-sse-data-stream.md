...
its list. Assert: with the grid mounted, a `media:changed` causes exactly one list refetch (now cheap —
...
On `media:changed` on a mounted grid → **revalidate the grid** (refetch the list). Not a per-item local
...
1. **Endpoint streams data events.** RED: an authed client on `GET /api/events/data` receives a
   `media:changed` frame when a scoped run commits; unauthed is rejected; `run:*` does **not** appear
   here. GREEN: SSE endpoint subscribing to the data family, reusing the P4 writer helper. REFACTOR.
2. **`useDataEvents` revalidates a mounted grid.** RED (mocked `EventSource`): dispatching
   `media:changed` triggers exactly one SWR revalidation of the media list. GREEN: hook subscribes and
...
3. **Resync on connect.** RED: on `open`/reconnect the grid revalidates once to seed a baseline. GREEN.
   REFACTOR.
4. **Whole-scope revalidation.** A `media:changed` carries no discriminator, so a mounted grid of either
   kind revalidates — over-fetch is safe, same philosophy as the cache. Segmented revalidation (skip a
   series grid for a movies-only change) is deferred to whenever the event gains a within-scope
   discriminator (see Phase 3 / `docs/intent/domain-event-bus-hardening.md`); not built here.
...
`/api/events/data` streams `media:changed` under auth, `useDataEvents` revalidates a mounted grid once
