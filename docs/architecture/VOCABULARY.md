...
| **MediaSource** | Role: owns a media collection — what *exists* + a canonical per-item id. Advertises `getMediaItems()`/`idOf()`, not `getMovies`/`getSeries`. Media-owned; provider connections are bound to it by adapters, never implement it directly. | [`server/modules/media/mediaSource.ts`](ref:path:server/modules/media/mediaSource.ts) |
...
| **MediaSourceFactory** | Resolves a `ContentType` to its active owner provider bound as a `MediaSource` (`OWNER_TYPE`: movie→Radarr, show→Sonarr). | [`server/modules/media/mediaSourceFactory.ts`](ref:path:server/modules/media/mediaSourceFactory.ts) |
