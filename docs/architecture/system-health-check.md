...
`server/health/systemHealthCheck.ts` — exported as a single async function called from
`server/index.ts` after `initDatabase()`. Each check is a named assertion. Critical failures
