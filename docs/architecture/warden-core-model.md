...
| **MediaEnricher** | Role: contributes metadata about media it does *not* own, joined by a logical key; `enrich(items)` decorates the canonical `MediaItem`, precedence resolved per field at write time. Built by Plex/Tautulli/Overseerr/TMDB. Spec: `docs/architecture/media-enricher-role.md`. | [`server/providers/roles.ts`](ref:path:server/providers/roles.ts) |
| **MediaActuator** | Role: exposes actions on addressable media and **owns its tasks** via `tasks(): ActuatorTask[]`. The sole authority for what tasks exist — a system without the role has no tasks. Spec: `docs/architecture/actuator-task-ownership.md`. | [`server/providers/roles.ts`](ref:path:server/providers/roles.ts) |
| **BaseProviderConnection** | The shared HTTP-client + config base. A *connection* base, not a role/metadata contract. | [`server/providers/baseProviderConnection.ts`](ref:path:server/providers/baseProviderConnection.ts) |
| **MediaQuerySpec** | The persistable, source-less core: `{ contentType, sources }`. Shared by the two below. | [`server/services/mediaQueryEngine.ts`](ref:path:server/services/mediaQueryEngine.ts) |
...
| **MediaItemSet** | The transient result of resolving/evaluating a source: normalized items. | [`server/providers/mediaSource.ts`](ref:path:server/providers/mediaSource.ts) |
| **ActuatorTask / ActuatorTaskDescriptor** | The role's task shapes: a descriptor (`id, label, destructive, affects?`) and the descriptor + instance-bound `run(ids)`. Declared by `MediaActuator.tasks()` on the configured instance — no type-keyed table. | [`server/providers/roles.ts`](ref:path:server/providers/roles.ts) |
| **MediaRule / MediaRuleDescriptor** | The queryable predicate shapes: `MediaRule` pairs a `predicate` with `{ key, label, contentTypes, dataType, sourceProviders, required }`; `MediaRuleDescriptor` is its JSON-honest, predicate-free wire projection. `MEDIA_RULES` is the single authority — no client-side rule catalogue. Client derives its filter controls from `GET /api/filter-fields`'s provider-gated `MediaRuleDescriptor[]` (`useMediaRules`), scoping state by content type (`shared`/`movie`/`show`) since the registry intentionally reuses keys (`tagIds`, `qualityProfileIds`, `genres`) across movie/show. | [`server/utils/filterRegistry.ts`](ref:path:server/utils/filterRegistry.ts) |
...
| **MediaSourceFactory** | Resolves a `ContentType` to its active owner provider bound as a `MediaSource`. | [`server/providers/mediaSourceFactory.ts`](ref:path:server/providers/mediaSourceFactory.ts) |
| **Normalized\*** | Canonical domain shapes (`NormalizedMovie`/`NormalizedShow`) predicates run over. | [`server/providers/normalizeMedia.ts`](ref:path:server/providers/normalizeMedia.ts) |
...
## Tasks & execution
[`AutomationExecutor`](ref:label:AutomationExecutor) runs a task against the ids a query matches. **The `MediaActuator` role owns its tasks** ([`server/providers/roles.ts`](ref:path:server/providers/roles.ts)): `tasks(): ActuatorTask[]` declares them on the configured instance, each an `ActuatorTaskDescriptor { id, label, destructive, affects? }` plus a `run(ids)` **bound to that instance — no cast**. The executor binds the provider by `automation.provider.id` and dispatches via `source.tasks().find(t => t.id === taskId)`; `system` automations dispatch to [`SystemTaskRunner`](ref:label:SystemTaskRunner) (identity/enrichment jobs), kept separate because they are internal jobs, not actuator tasks. Scheduling via `croner`. Tasks are the surface of the role, not a property of every provider — an enricher-only type (TMDB) declares the role and so contributes none. **Enablement is per instance** (`settings.enabledTasks`, default off, via `readEnabledTaskIds`): `automationService.create` and the executor both refuse a task not enabled on *that instance*, so an un-enabled task can neither persist nor run. `GET /api/providers/tasks` serves **instance-keyed** availability (`{ providerId, type, tasks: [{…descriptor, enabled}] }`), non-actuators absent. There is no type-keyed task table — the role on the instance is the only server-side declaration. The client catalogue ([`src/lib/provider-registry.ts`](ref:path:src/lib/provider-registry.ts)) is a known stale duplicate until Stage 2 inverts it. As-built role-owned model: `docs/architecture/actuator-task-ownership.md`.
